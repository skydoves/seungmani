...
public abstract class MeResponseCallback extends UserProfileResponseCallback {
     ...
     */
    public abstract void onSuccess(final UserProfile userProfile);
     ...
     */
    public abstract void onSessionClosedFailure(final APIErrorResult errorResult);
     ...
     */
    @Override
    public void onSuccessUserProfile(final UserProfile userProfile) {
        if(userProfile == null)
    ...
    /**
     * {@link com.kakao.auth.http.HttpResponseHandler}를 구현한 것으로 사용자정보 요청 전 또는 요청 중에 세션이 닫혀 로그인이 필요할 때 호출된다.
    ...
    public void onHttpSessionClosedFailure(final APIErrorResult errorResult) {
        Logger.d("MeResponseCallback : session is closed before requesting user info. errorResult = " + errorResult);
        onSessionClosedFailure(errorResult);
    }

    /**
     * {@link com.kakao.auth.http.HttpResponseHandler}를 구현한 것으로 사용자정보 요청 중에 서버에서 요청을 수행하지 못했다는 결과를 받았을 때 호출된다.
    ...
    public void onHttpFailure(final APIErrorResult errorResult) {
        if (errorResult.getErrorCode() == ErrorCode.NOT_REGISTERED_USER_CODE) {
            Logger.d("MeResponseCallback : " + errorResult);
            onNotSignedUp();
        } else {
     ...
     */
    public void onError(final String msg, final APIErrorResult errorResult) {
