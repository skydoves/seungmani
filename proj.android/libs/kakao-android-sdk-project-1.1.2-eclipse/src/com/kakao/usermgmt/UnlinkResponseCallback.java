...
public abstract class UnlinkResponseCallback extends UserResponseCallback {
     ...
     * {@link com.kakao.auth.ErrorCode#NOT_REGISTERED_USER_CODE}, <br/>
    ...
    public void onSuccessUser(final User user) {
        if (user == null || user.getId() <= 0)
            ...
            Logger.d("UnlinkResponseCallback : unlinked successfully. user = " + user);
    ...
    /**
     * {@link com.kakao.auth.http.HttpResponseHandler}를 구현한 것으로 unlink 요청 전 또는 요청 중에 세션이 닫혀 로그인이 필요할 때 호출된다.
    ...
    public void onHttpSessionClosedFailure(final APIErrorResult errorResult) {
        Logger.d("UnlinkResponseCallback : session is closed before requesting unlink. errorResult = " + errorResult);
        onSessionClosedFailure(errorResult);
    }

    /**
     * {@link com.kakao.auth.http.HttpResponseHandler}를 구현한 것으로 unlink 요청 중에 서버에서 요청을 수행하지 못했다는 결과를 받았을 때 호출된다.
